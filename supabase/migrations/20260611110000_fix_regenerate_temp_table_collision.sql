-- Fix: deleting a grocery list with 2+ recipes failed with
--   relation "temp_ingredient_aggregation" already exists
--
-- The grocery_list_recipes AFTER DELETE trigger fires once per row, so a
-- cascade delete runs regenerate_grocery_list_items() several times inside
-- ONE transaction. Its CREATE TEMP TABLE ... ON COMMIT DROP tables only drop
-- at commit, so the second invocation collides. Drop them up front so the
-- function is safe to re-run within a transaction.
--
-- This function is already ported to TypeScript (src/server/) and is slated
-- to be dropped entirely once the soak period ends; this patch just keeps the
-- legacy trigger path working until then.
